...
int OSPL_MAIN (int argc, char *argv[])
{
  ...
  //create type
  IoTDataTypeSupport_var st = new IoTDataTypeSupport();
  ...
  DataRiverChannelListener *myListener = new DataRiverChannelListener();
  myListener->m_IoTDataReader = IoTDataDataReader::_narrow(dreader.in());
  checkHandle(myListener->m_IoTDataReader.in(), "IoTDataReader::_narrow");

  cout << "=== [ListenerDataSubscriber] set_listener" << endl;
  DDS::StatusMask mask =
           DDS::DATA_AVAILABLE_STATUS | DDS::REQUESTED_DEADLINE_MISSED_STATUS;
  myListener->m_IoTDataReader->set_listener(myListener, mask);
  ...
  while (!myListener->m_closed && count < 5500 ){
  ...
  //cleanup
  mgr.deleteReader(myListener->m_IoTDataReader.in ());
